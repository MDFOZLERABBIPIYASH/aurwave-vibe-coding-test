...
- **Phase 14 — Vercel Analytics + Speed Insights (`@vercel/analytics`, `@vercel/speed-insights`):** wired in `src/app/layout.tsx`. Both auto-register when the project is deployed to Vercel; no env vars or API keys required. Components are no-ops locally and outside Vercel, so there is zero runtime cost in other environments. Bundle impact: ~0 KB (shared chunk unchanged at 103 kB; per-route page weight unchanged).
...
- **CI fix — regenerated `package-lock.json`:** the previous lock file was missing ~3000 lines of entries for the Sentry / Vercel Analytics / Speed Insights / `@testing-library/dom` installs. `npm ci` (used by the CI workflow) is strict and was failing every run since the Sentry install in commit `b9dc4ff`. The lock file is now in sync with `package.json`; `npm ci` in a clean directory succeeds and produces a working build.
- **CI fix — `tests/unit/lockfile.test.ts` (new):** adds two tests that fail the suite the next time the lock file drifts. The first runs `npm install --package-lock-only --dry-run` and asserts the output contains "up to date". The second asserts `lockfileVersion: 3`. **33/33 unit tests pass** (was 31).
- **CI fix — `.github/workflows/ci.yml`:** switched `npm ci` to `npm ci --legacy-peer-deps` in all three jobs. The Sentry / Vercel Analytics / Speed Insights install pulls in a transitive Vite 8 / SvelteKit peer dependency from `@vercel/analytics` that npm refuses to resolve without the legacy flag. The lockfile test in `tests/unit/lockfile.test.ts` uses the same flag for consistency. Also added diagnostic output (Node version, npm version, lockfile version + line count) to the install step so future failures are easier to triage from the GitHub UI.
- **CI fix — `.github/workflows/ci.yml` (e2e job):** added `npm run build` to the e2e job. Each GitHub Actions job runs in a fresh runner, so the `build` job's `.next` directory is not available to the e2e job — the `next start` command in `playwright.config.ts` was failing with "Could not find a production build in the '.next' directory". This was the actual cause of the e2e step failure; the diagnostic output is also kept for future triage.
